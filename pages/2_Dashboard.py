...
st.set_page_config(page_title="Procurement Dashboard", layout="wide", initial_sidebar_state="expanded")
...
st.caption("Ask follow-up questions about the procurement data shown above. Only procurement-related questions are answered.")
...
if user_q:
    ...
    with st.chat_message("assistant"):
        with st.spinner("Analyzing..."):
            try:
                prompt = f"{full_context}\n\nUser question: {user_q}\n\nAnswer in under 80 words. Be specific to the data shown. If the question is not about procurement, respond with the guardrail message."
                safe = prompt.replace("'", "''")
                result = run_query_df(f"SELECT SNOWFLAKE.CORTEX.COMPLETE('{CORTEX_MODEL}', '{safe}') AS S")
                if not result.empty:
